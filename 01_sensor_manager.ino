...
class SensorManager {
  ...
  public:
    ...
    SensorManager() : sensorCount(2), lastReadTime(0) {
      // Capteur Température (thermistance)
    ...
    void begin() {
      // Configurer  capteur lumière (pin 36)
      pinMode(LIGHT_SENSOR_PIN, INPUT);
      
      // Configurer température (pin 39)
      pinMode(TEMP_SENSOR_PIN, INPUT);
      
      // Configuration ADC(Analog to Digital Converter) : atténuation 11dB pour plage 0-3.3V
  ...
  private:
    ...
    float readTemperature() {
      // Faire plusieurs lectures et calcul de la moyenne pour réduire le bruit
      ...
      if (steinhart < -40.0 || steinhart > 80.0) {
        ...
        return TEMP_NOMINAL; // Retourner valeur par défaut si aberrante (25°)
      }
    ...
    float readLightLevel() {
      // Faire plusieurs lectures et calculer la moyenne pour réduire le bruit
      const int NUM_SAMPLES = 5;
      float sum = 0;
